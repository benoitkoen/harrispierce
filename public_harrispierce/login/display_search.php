...
if(isset($_POST['submitSearch'])){//to run PHP script on submit
 
    if(!empty($_POST['check_list_search'])){ //Si des journaux ont ete choisis et un keyword a bien ete donne
 
        
        $checked_count = count($_POST['check_list_search']);  //check_list_search[]
        ...
        $topic = $dbc->real_escape_string($_POST['topicSearch']);
        
        foreach($_POST['check_list_search'] as $selected){   //check_list_search[] 
            ...
            $sql .= "INSERT INTO search_loggedin (date, email, journal, inputdate, searchlimit, topic) VALUES ('$date', '$email',  '$journal', '$searchDate', '$searchLimit', '$topic')";
                    ...
                    if ($counter !== 0) { //si y a deja des sections appendee a div
                        ...
                        if ($dom->getElementById($section) !== NULL){
                            ...
                            if($dom->getElementById($journal.$section) !== NULL){
                                ...
                                $newDate = $dom->createElement('p', $row["date"]);
