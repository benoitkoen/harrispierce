    ...
    if ($journal === 'Wall Street Journal'){ //IMAGES
        if ($url === 'https://www.wsj.com/news/world'
            ...
            foreach($html->find('.hed-summ') as $hed){
                ...
                if ($hed->find('div[class=text-wrapper]', 0) !== NULL){
                    array_push($hrefs, urlencode($hed->find('.hedgroup', 0)->find('h2, h3', 0)->find('a', 0)->getAttribute('href'))); 
                ...
                if ($hed->find('div[class=text-wrapper]', 0) !== NULL){
                    ...
                    if ($hed->find('div[class=media-wrapper]', 0) !== NULL){
                        ...
                        if ($hed->find('div[class=media-wrapper]', 0)->children(0)->getAttribute('class') === 'video-wrapper'){
                            ...
                            //echo 'image'."<br>";
                            array_push($images, urlencode($hed->find('div[class=media-wrapper]', 0)->children(0)->find('a[class=image]', 0)->find('img', 0)->getAttribute('data-src')));
                        }
            ...
            foreach($html->find('h3[class=wsj-headline dj-sg wsj-card-feature heading-3 locked], h3[class=wsj-headline dj-sg wsj-card-feature heading-3 unlocked], h3[class=wsj-headline dj-sg wsj-card-feature heading-2 locked]') as $hed){
                ...
                if ($hed->find('a', 0) !== NULL){
                    array_push($hrefs, urlencode($hed->find('a', 0)->getAttribute('href'))); 
                }
                ...
                if ($hed->find('a', 0) !== NULL){ 
                    ...
                    if ($hed->nextSibling()->getAttribute('class') === 'right wsj-card-feature wsj-card-media Image'){
                        ...
                        array_push($images, urlencode($hed->nextSibling()->find('div[class=wsj-image wsj-card-media-item]', 0)->find('meta', 0)->getAttribute('content')));
        ...
        if ($url === 'https://www.nytimes.com/section/technology'
           || $url === 'https://www.nytimes.com/section/world') {
            ...
            foreach($html->find('ol[class=story-menu theme-stream initial-set]', 0)->find('li') as $hed){
                ...
                if ($hed->find('article[class=story theme-summary]', 0) !== NULL){
                    array_push($hrefs, urlencode($hed->find('article[class=story theme-summary]', 0)->find('div[class=story-body]', 0)->find('a[class=story-link]', 0)->getAttribute('href'))); 
                }
                ...
                if ($hed->find('article[class=story theme-summary]', 0) !== NULL){
                    ...
                    array_push($images, urlencode($hed->find('article[class=story theme-summary]', 0)->find('div[class=story-body]', 0)->find('a[class=story-link]', 0)->find('div[class=wide-thumb]', 0)->find('img', 0)->getAttribute('src'))); 
            ...
            foreach($html->find('div[class=story]') as $hed){
                ...
                if ($hed->find('h3', 0) !== NULL){
                    array_push($hrefs, urlencode($hed->find('h3', 0)->find('a', 0)->getAttribute('href'))); 
                }
                ...
                if ($hed->find('h3', 0) !== NULL){
                    ...
                    if ($hed->find('div[class=thumbnail]', 0) !== NULL){
                        array_push($images, urlencode($hed->find('div[class=thumbnail]', 0)->find('a', 0)->find('img', 0)->getAttribute('src')));   
                    }
                }
            }
        }
            ...
            foreach($html->find('li[class=o-teaser-collection__item o-grid-row]') as $hed){
                ...
                if (
                    ...
                    $hed->find('p[class=o-teaser__standfirst]', 0) !== NULL){
                        ...
                        array_push($hrefs, urlencode('https://www.ft.com' . $hed->find(('a.js-teaser-heading-link'), 0)->getAttribute('href'))); 
                }
                ...
                if (
                    ...
                    $hed->find('p[class=o-teaser__standfirst]', 0) !== NULL){
                        ...
                        array_push($images, urlencode(explode(" ", $hed->find(('div[class=o-teaser__image-container js-teaser-image-container]'), 0)->find('a', 0)->find('div[class=n-image-wrapper o-teaser__image-placeholder n-image-wrapper--lazy-loading n-image-wrapper--placeholder]', 0)->find(('img'), 0)->getAttribute('data-srcset'))[0]));
            ...
            foreach($html->find('article[class=article-small article-medium]') as $hed){
                ...
                if ($hed->find('[class=titre]', 0) !== NULL){
                    array_push($hrefs, urlencode($hed->find('[class=titre]', 0)->find('a', 0)->getAttribute('href'))); 
                }
                
                if ($hed->find('[class=titre]', 0) !== NULL){
                    array_push($images, urlencode('https://www.lesechos.fr' . $hed->find('figure', 0)->find('a', 0)->find('picture', 0)->find('img', 0)->getAttribute('srcset')));
...
foreach($journalList as $journal){
    ...
    foreach ($urls as $section => $url) {
        ...
        for ($i = 0; $i < count($res[0]); $i++){
            ...
            $href = mysqli_real_escape_string($dbc, $res[1][$i]);
